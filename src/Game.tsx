...
const Game: React.FC = () => {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      try {
        const response = await axios.get(`https://100bwc-xi.vercel.app//scores`, { params: { roomName } });
  ...
  const handleCreateScore = async () => {
    ...
    if (scoreName && !isNaN(defaultValue)) {
      try {
        const response = await axios.post("https://100bwc-xi.vercel.app//create-score", { roomName, scoreName, defaultValue });
        if (response.data.success) {
  ...
  const handleDeleteScore = async (scoreName: string) => {
    try {
      const response = await axios.post("https://100bwc-xi.vercel.app//delete-score", { roomName, scoreName });
      if (response.data.success) {
        setScores((prevScores: any) => {
  ...
  useEffect(() => {
    ...
    newSocket.on("updateHands", async () => {
      ...
      try {
        const response = await axios.get(`https://100bwc-xi.vercel.app//lobbies/${roomName}/hands/${username}`);
        const handUrls = response.data.hand.map((card: string) => `https://100bwc-xi.vercel.app/${card}`);
    ...
    const fetchHandCards = async () => {
      try {
        const response = await axios.get(`https://100bwc-xi.vercel.app//lobbies/${roomName}/hands/${username}.json`);
    ...
    const fetchPlayers = async () => {
      try {
        const response = await axios.get(`https://100bwc-xi.vercel.app//lobbies/${roomName}/players`);
        if (response.data.success) {
  ...
  const handleDeckMenuOptionClick = (option: string) => {
    ...
    if (socket) {
      if (option === "Draw") {
        axios.post("https://100bwc-xi.vercel.app//peek-deck", { roomName, deckName: "main", count: 1 })
          .then(response => {
        ...
        if (count > 0) {
          axios.post("https://100bwc-xi.vercel.app//peek-deck", { roomName, deckName: "main", count })
            .then(response => {
              if (response.data.success) {
                setPeekCards(response.data.cards.map((card: string) => `https://100bwc-xi.vercel.app/${card}`));
  ...
  const handleAdditionalDeckButtonClick = async () => {
    ...
    if (!additionalDeckMenuVisible) {
      try {
        const response = await axios.get(`https://100bwc-xi.vercel.app//lobbies/${roomName}/decks`);
  ...
  const handleAdditionalDeckMenuOptionClick = (deckName: string, option: string) => {
    ...
    if (socket) {
      if (option === "Draw") {
        axios.post("https://100bwc-xi.vercel.app//peek-deck", { roomName, deckName, count: 1 })
          .then(response => {
            if (response.data.success && response.data.cards.length > 0) {
        ...
        const count = parseInt(prompt("Enter the number of cards to view:")||"", 10);
        if (count > 0) {
          axios.post("https://100bwc-xi.vercel.app//peek-deck", { roomName, deckName, count })
            .then(response => {
              if (response.data.success) {
                setPeekCards(response.data.cards.map((card: string) => `https://100bwc-xi.vercel.app/${card}`));
                setShowPeekPopup(true);
  ...
  useEffect(() => {
    if (socket) {
      socket.on("cardDrawnToField", async (cardUrl) => {
        const flippedCardUrl = "https://100bwc-xi.vercel.app//assets/FLIPPED.png";
        const img = new Image();
  ...
  const handlePlayCard = async () => {
    if (socket && selectedCard) {
      const cardUrl = selectedCard.startsWith("/assets/") ? `https://100bwc-xi.vercel.app/${selectedCard}` : selectedCard;
      const img = new Image();
      img.src = cardUrl;
      img.onload = async () => {
  ...
  const handleUploadCard = async () => {
    ...
    fileInput.onchange = async (event: any) => {
      ...
      if (file) {
        ...
        try {
          const response = await axios.post("https://100bwc-xi.vercel.app//upload-card-image", formData, {
            headers: {
  ...
  const handleFlip = async () => {
    if (socket && selectedKonvaCard) {
      ...
      const cardUrl = isFlipped ? selectedKonvaCard.data?.originalCard : selectedKonvaCard.data?.originalCard || "https://100bwc-xi.vercel.app//assets/FLIPPED.png";
      if (!cardUrl) {
  ...
  const handlePlayAdditionalCard = (additionalCard: string) => {
    if (socket && selectedKonvaCard) {
      ...
      if (additionalCard.startsWith("https://100bwc-xi.vercel.app/")) {
        additionalCardUrl = additionalCard;
      } else if (deckName) {
        additionalCardUrl = `https://100bwc-xi.vercel.app//decks/${deckName}/additional/${additionalCard}`;
      } else {
  ...
  const handleSaveEdit = async () => {
    if (socket && selectedKonvaCard) {
      ...
      try {
        await axios.post("https://100bwc-xi.vercel.app//save-konva-card", {
          roomName,
          card: updatedCard,
        });
  ...
  const handleShuffleInDeckSelect = async (deckName: string) => {
    if (socket && selectedKonvaCard) {
      const formattedCardUrl = selectedKonvaCard.src.startsWith("https://100bwc-xi.vercel.app/") ? selectedKonvaCard.src.replace("https://100bwc-xi.vercel.app/", "") : selectedKonvaCard.src;
      try {
        const response = await axios.post("https://100bwc-xi.vercel.app//shuffle-in-card", {
          roomName,
  ...
  const handleScoreChange = async (scoreName: string, playerName: string, newValue: number) => {
    if (socket) {
      try {
        ...
        await axios.post("https://100bwc-xi.vercel.app//update-score", { roomName, scoreName, playerName, newValue });
        logAction({
          player: username,
  ...
  const handleViewCards = async () => {
    if (selectedPlayer) {
      ...
      if (count > 0) {
        ...
        try {
          const response = await axios.get(`https://100bwc-xi.vercel.app//lobbies/${roomName}/hands/${selectedPlayer}`);
          const handUrls = response.data.hand.map((card: string) => `https://100bwc-xi.vercel.app/${card}`);
  ...
  const handleToHand = async () => {
    if (socket && selectedKonvaCard) {
      try {
        const formattedCardUrl = selectedKonvaCard.src.startsWith("https://100bwc-xi.vercel.app/") ? selectedKonvaCard.src.replace("https://100bwc-xi.vercel.app/", "") : selectedKonvaCard.src;
        const response = await axios.post("https://100bwc-xi.vercel.app//add-card-to-hand", {
          roomName,
          username,
          cardUrl: formattedCardUrl,
        });
    ...
    try {
      ...
      if (response.data.success) {
        ...
        for (const player of players) {
          const handResponse = await axios.get(`https://100bwc-xi.vercel.app//lobbies/${roomName}/hands/${player}`);
  ...
  return (
    <>
      ...
      {chatVisible && (
        <img
          src="https://100bwc-xi.vercel.app//assets/COOKIE.png"
          alt="Cookie"
        ...
        <div style={styles.additionalPopup}>
          <div style={styles.additionalCardContainer}>
            ...
            {selectedKonvaCard.data?.additional?.map((additionalCard: string, index: number) => {
              ...
              const deckNameMatch = mainCardUrl.match(/\/decks\/([^/]+)\//);
              const deckName = deckNameMatch ? deckNameMatch[1] : null;
              const additionalCardUrl = deckName ? `https://100bwc-xi.vercel.app//decks/${deckName}/additional/${additionalCard}` : null;
