...
const LobbyScreen: React.FC = () => {
  ...
  const handleSelectDeck = async (deck: any) => {
    if (deck.isPublic) {
      ...
      if (password) {
        const response = await axios.post("http://localhost:3000/validate-usage-password", {
  ...
  useEffect(() => {
    const fetchDecks = async () => {
      ...
      const decksWithInfo = await Promise.all(response.data.decks.map(async (deck: any) => {
        const deckInfoResponse = await axios.get("http://localhost:3000/deck-info", {
          ...
          <ul style={styles.deckSearchList}>
            {decks.map((deck, index) => (
              <li key={deck.deckName} style={{ ...styles.deckSearchItem, fontWeight: index % 2 === 0 ? 'bold' : 'normal' }}>
                ...
                <img src={`http://localhost:3000/decks/${deck.deckName}/important/${deck.thumbnail}`} alt="thumbnail" style={styles.deckThumbnail} />
                <span style={styles.deckNameText}>{deck.deckName}</span>
      ...
      {selectedDeckInfo && (
        <div style={styles.deckInfoOverlay} onClick={handlePopupClick}>
          <div style={styles.deckInfoPopup}>
            ...
            <img src={`http://localhost:3000/decks/${selectedDeckInfo.deckName}/important/${selectedDeckInfo.thumbnail}`} alt="thumbnail" style={styles.enlargedThumbnail} />
